...
var RecordComponent = (function () {
    function RecordComponent(clientApi, recordApi, route, router, dataSampleApi, recordBuffer) {
    ...
    RecordComponent.prototype.getRecord = function () {
        ...
        this.route.params.forEach(function (params) {
            ...
            _this.recordBuffer.getRecordData(_this.recordId)
                .subscribe(function (localDbRecord) {
                console.log("record from local db");
                console.log(localDbRecord);
                var filter = { include: ["client"] };
                if (localDbRecord != undefined) {
                    filter.fields = { data: false };
                }
                console.log("I am here");
                debugger;
                _this.recordApi.findById(_this.recordId, filter).subscribe(function (_record) {
                    ...
                    console.dir(_record);
                    if (localDbRecord != undefined) {
                        _this.dataStr = localDbRecord.data;
                        _this.recordBuffer.put(_record)
                            .subscribe(function (res) { console.log("sucess"); }, function (err) { console.log(err); });
                    }
                    else {
                        _this.dataStr = _record.data;
                        _this.recordBuffer.add(_record)
                            .subscribe(function (res) { console.log("sucess"); }, function (err) { console.log(err); });
                    }
